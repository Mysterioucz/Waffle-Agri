import { CostBreakdown, Crop, FarmActivity, ResourceUsage } from "@/types";
import { CROP_GROWTH_DAYS, POINTS } from "./constants";
...
export function calculateGrowthProgress(crop: Crop): number {
    ...
    const daysPassed = Math.floor(
        (Date.now() - new Date(crop.plantingDate).getTime()) /
            (1000 * 60 * 60 * 24),
    );
